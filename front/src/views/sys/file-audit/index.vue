...
import { getGetFileAuditList, deleteGetFileAudit,passOrDis } from "./api.js";
// 根据你的实际添加编辑组件位置路径修改
...
export default {
  ...
  data() {
    return {
      ...
      },
      {
        ...
        width: 150,
        render: (h, params) => {
          if(params.row.fileType == 0 && params.row.step1 != 1 || params.row.fileType == 1 && params.row.step4 != 1) {
            return h("div", [
              h(
                "a",
                {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
      getGetFileAuditList(this.searchForm).then(res => {
        console.log(res);
        this.loading = false;
        if (res.success) {
