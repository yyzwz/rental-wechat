<template>
	<view class="visit">
		<view class="notify-list">
			<view v-for="(item,index) in auditDanList"  class="row" @tap="navToDetail($event)" :data-danid="item.id">
				<view class="carrier">
					<view class="notify-wrapper">
						<view class="title in1line">
							{{item.sqr}}的用车申请 -- 
							<span v-show="item.status==0">尚未审核</span>
							<span v-show="item.status==1">已通过</span>
							<span v-show="item.status==2">未通过</span>
						</view>
						<view class="content in2line">车牌号{{item.car}},车辆所有人{{item.carOwner}},驾驶人{{item.jcr}},开始时间{{item.kssj}},结束时间{{item.jssj}},始发地{{item.ccdd}},目的地{{item.mdd}}</view>
	...
	export default {
		...
		methods: {
			navToDetail(e) {
				console.log(e);
				uni.navigateTo({
					url:'./detail?id=' + e.currentTarget.dataset.danid
				})
			},
			getAuditDanList() {
				var that = this;
